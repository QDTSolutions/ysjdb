	...
	class ezSQLcore
	{
		...
		*/
		...
		function get_row($query=null,$output=OBJECT,$y=0)
		{
			...
			if ( $output == OBJECT )
			{
				return !empty($this->last_result[$y]) ? $this->last_result[$y] : null;
			}
			// If the output is an associative array then return row as such..
			elseif ( $output == ARRAY_A )
			{
				return !empty($this->last_result[$y]) ? get_object_vars($this->last_result[$y]) : null;
			}
			// If the output is an numerical array then return row as such..
			elseif ( $output == ARRAY_N )
			{
				return !empty($this->last_result[$y]) ? array_values(get_object_vars($this->last_result[$y])) : null;
			}
		...
		*/
		...
		function get_col($query=null,$x=0)
		{
			...
			$j = !empty($this->last_result) ? count($this->last_result) : 0;
			for ( $i=0; $i < $j; $i++ )
			{
